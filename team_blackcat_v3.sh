...
# ---------------- Cek NIK via Nomor Telepon (Leak checker) ----------------
cek_nik_via_phone(){
  clear
  echo -e "${BLU}=== CEK NIK VIA NOMOR TELEPON (Leak Checker) ===${RST}"
  read -rp "Masukkan nomor telepon (contoh +6281234567890 atau 081234567890): " ph
  [[ -z "$ph" ]] && { echo "Nomor kosong."; pause; return; }

  # Normalisasi basic: buat var +62 and no plus
  ph_raw="$ph"
  ph_plus="$(echo "$ph_raw" | sed 's/^0/+62/; s/[^0-9+]//g')"
  ph_no_plus="$(echo "$ph_plus" | sed 's/^+//')"

  echo -e "${YEL}Mencari jejak kebocoran yang menyebut 'NIK' bersama nomor: ${ph_plus}${RST}"
  UA="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/125 Safari/537.36"

  # list of search URLs to check (we only generate safe search links)
  queries=(
    "https://www.google.com/search?q=%22NIK+${ph_no_plus}%22"
    "https://duckduckgo.com/?q=%22NIK+${ph_no_plus}%22"
    "https://www.google.com/search?q=%22${ph_no_plus}+NIK%22"
    "https://duckduckgo.com/?q=%22${ph_no_plus}+NIK%22"
    "https://pastebin.com/search?q=${ph_no_plus}"
    "https://search.strikepoint.io/?q=${ph_no_plus}"
    "https://www.google.com/search?q=%22${ph_no_plus}+nik%22"
    "https://www.bing.com/search?q=%22NIK+${ph_no_plus}%22"
    "https://search.yahoo.com/search?p=%22NIK+${ph_no_plus}%22"
    "https://www.scamcallfighters.com/search/?s=${ph_no_plus}"
  )

  printf "%-4s | %-7s | %s
" "No" "Status" "URL"
  printf "%-4s-+-%-7s-+-%s
" "$(printf '%.0s-' {1..4})" "$(printf '%.0s-' {1..7})" "$(printf '%.0s-' {1..60})"
  ...
  for q in "${queries[@]}"; do
    status="SKIP"
    if [[ $has_curl -eq 1 ]]; then
      code=$(curl -A "$UA" -m 8 -s -o /dev/null -w "%{http_code}" "$q" || echo "000")
      status="$code"
    fi
    printf "%-4s | %-7s | %s
" "$i" "$status" "$q"
  ...
  echo
  echo "Keterangan: script hanya menampilkan link & status HTTP (indikator)."
  echo "Buka link yang relevan di browser untuk verifikasi (jika ada indikasi bocor, laporkan ke pihak yang berwenang)."
  pause
}
...
main() {
  while true; do
    ...
    echo -e "${GRN}│  ${YEL}[8] Cek NIK via Nomor Telepon     ${GRN}│${RST}"
